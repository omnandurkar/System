...
export function AdminTaskRow({ task, dragHandleProps }) {
    ...
    async function handleSave() {
        setLoading(true);
        await updateTask(task.id, title, parseInt(exp), mask);
        setLoading(false);
        setIsEditing(false);
    }
        ...
        return (
            <div className="flex flex-col gap-3 rounded bg-muted/50 p-3">
                <div className="flex items-center gap-4">
                    <input
                        className="flex-1 rounded border border-input bg-background px-3 py-1 text-sm text-white"
                        value={title}
                ...
                <div className="flex items-center gap-1.5 ml-1">
                    <span className="text-xs text-muted-foreground mr-2 font-mono">ACTIVE DAYS:</span>
                    {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((char, idx) => (
                        <button
                            key={idx}
                            onClick={() => toggleDay(idx)}
                            className={cn(
                                "cursor-pointer flex h-6 w-6 items-center justify-center rounded border text-xs font-mono transition-colors",
                                mask[idx] === '1'
    ...
    return (
        <div className="flex items-center justify-between border-b border-border py-2 last:border-0 hover:bg-muted/20 bg-background group">
            <div className="flex items-center gap-2">
                <div
                    {...dragHandleProps}
                    className="cursor-grab active:cursor-grabbing p-1 text-muted-foreground opacity-50 hover:opacity-100 transition-opacity"
                >
...
export function AddTaskForm({ routineId }) {
    ...
    async function handleAdd() {
        ...
        setLoading(true);
        await addTask(routineId, title, parseInt(exp), mask);
    ...
    return (
        <div className="mt-4 flex flex-col gap-3 rounded border border-border bg-card/50 p-3 shadow-inner">
            ...
            <div className="flex items-center gap-4">
                <input
                    className="flex-1 rounded border border-input bg-background px-3 py-1 text-sm text-white focus:outline-none focus:ring-1 focus:ring-primary"
                ...
                <input
                    type="number"
                    className="w-20 rounded border border-input bg-background px-3 py-1 text-sm text-white focus:outline-none focus:ring-1 focus:ring-primary"
                    value={exp}
                    onChange={(e) => setExp(e.target.value)}
                />
            </div>
            ...
            <div className="flex items-center justify-between mt-1">
                <div className="flex items-center gap-1.5 ml-1">
                    <span className="text-[10px] text-muted-foreground mr-2 font-mono uppercase tracking-widest">Active Days:</span>
                    {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((char, idx) => (
                        <button
                            key={idx}
                            onClick={() => toggleDay(idx)}
                            className={cn(
                                "flex h-5 w-5 items-center justify-center rounded border text-[10px] font-mono transition-colors",
                                mask[idx] === '1'
                                    ? "bg-primary text-primary-foreground border-primary"
                                    : "bg-transparent text-muted-foreground border-border hover:border-muted-foreground"
                            )}
